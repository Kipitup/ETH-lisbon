...
import {ERC20Interface} from '../src/interfaces/ERC20Interface.sol';
...
contract FlashLoanTest is Test, TestUtils {
	...
	function setUp() public {
		...
		(success2, ) = WETH.call{value: 0}(
			abi.encodeWithSignature('transfer(address,uint256)', address(flashLoan), INITIAL_WETH)
		);
		require(success2, 'WETH transfer failed');

		// console2.log("balance before %s", ERC20Interface(WETH).balanceOf(address(flashLoan)));

		// We approve the flash loan contract to spend our WETH.
		flashLoan.approve(WETH_AAVE);

		// We add liquidity.
		(uint256 amountA, uint256 amountB, uint256 liquidity) = flashLoan.addLiquidity(
			WETH,
			WETH_AAVE,
			5 ether,
			100 ether,
		...
		// console2.log("amountA %s and %s and %s", amountA, amountB, liquidity);
	...
	function testWethFlashLoan() public {
		...
		console2.log('----------------------------------------------------------------');
		console2.log('-                        START FLASH LOAN                      -');
		console2.log('----------------------------------------------------------------');
		console2.log('\n\n');
		console2.log('flash loan taken');
		console2.log('\n\n');
		flashLoan.flashloan(WETH_AAVE, 1, 2003);
		...
		// console2.log("balance after flashloan %s", ERC20Interface(WETH_AAVE).balanceOf(address(flashLoan)));
